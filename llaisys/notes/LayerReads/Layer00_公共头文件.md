...
**双下划线 `__` 或以下划线+大写字母开头的标识符被保留给编译器/标准库使用**
...
**`_WIN32` 是 Windows 平台下编译器预定义的宏**

`__export` 的作用是**标记 `tensorCreate` 为动态库的公开 API**，使得编译成 `.dll` / `.so` 后，外部程序能链接并使用这个函数。方便上层语言（如Python进行调用）

**动态库（Dynamic Library）：是一种在程序运行时才被加载到内存并链接的代码库，它的主要特点是与可执行文件分离，多个程序可以共享同一个库文件。**

**符号导出宏：在 C/C++ 动态库开发中，用于控制函数、变量、类等符号在库外是否可见的预处理宏。**
...
`__cplusplus` 是 C++ 编译器**预定义**的宏

`extern "C"` 是 **C++ 中的一个链接指示符**，用于告诉 C++ 编译器：被修饰的代码应该**按照 C 语言的方式进行编译和链接**。核心作用是解决 **C++ 与 C 混合编程时的符号命名问题**。起了个别名叫`__C`
...
`LLAISYS_DEVICE_TYPE_COUNT` 是枚举中的一个特殊成员（没有显式赋值，自动取前一个值加 1 ），它的值**等于当前枚举中有效设备类型的数量**
...
内存拷贝方向枚举：（主机Host、设备Device）×（主机Host、设备Device）一共四种
...
**全大写、不加前导下划线的名称**（如 `LLAISYS_TENSOR_H`）是用户程序的安全命名空间，不会与编译器内部标识符冲突。
